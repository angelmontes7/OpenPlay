...
import NoParticipantsModal from "@/components/NoParticipants";
...
const Wagers = () => {
  ...
  const handleCloseWager = (wagerDetails: any) => {
    ...
    // If only one participant, show NoParticipantsModal
    if (numberOfParticipants === 1) {
      setNoParticipantsModalVisible(true);
  ...
  return (
    <View className="flex-1 bg-gray-900">
                          ...
                          {activeTab === "Active" && (
                            ...
                            >
                              <Text className="text-white font-bold text-sm">
                                {item.amount_of_participants === 1 ? "Close Wager" : "Mark as Finished"}
                              </Text>
                            </TouchableOpacity>
                          )}
      ...
      {currentView === "create" && (
        ...
        />
        <NoParticipantsModal
          isVisible={noParticipantsModalVisible}
          onClose={() => setNoParticipantsModalVisible(false)}
          selectedWager={selectedWager}
          userId={user?.id || ""}
          onConfirmed={() => {
            setDisputeModalVisible(false);
